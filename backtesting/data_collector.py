...
def collect_all(client: typing.Union[BinanceClient, FtxClient], exchange: str, symbol: str):
    ...
    if oldest_ts is None:
        ...
        if len(data) == 0:
            ...
            logger.info("%s %s: Collected %s initial data from %s %s" %
                        (exchange, symbol, len(data), ms_to_dt(data[0][0]), ms_to_dt(data[-1][0])))
    ...
    while True:
        ...
        if data is None:
            time.sleep(4)  # Pause in case error occurs during request
            continue
    ...
    while True:
        ...
        if len(data) == 0:
            logger.info("%s %s: Stopped older data collection because no data was found beforfe %s" %
