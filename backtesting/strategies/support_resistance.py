...
class SupportResistance(NativeStrategy):
    ...
    def _backtest(self, df: pd.DataFrame) -> Tuple[float, float]:
        self._compute_history(df)

        return self.pnl, self.max_drawdown

    def _trade_history(self, df) -> pd.DataFrame:
        return self._compute_history(df)

    def plot(self):
        mpf.plot(self.df, type="candle", style="charles", alines=dict(alines=self.levels["resistances"] + self.levels["supports"]))
        plt.show()
